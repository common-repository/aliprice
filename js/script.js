					...
					var output = '<div class="advanced-settings import-settings">';
						...
						$.each(response.result.products, function(i,str){
							...
							output += ' <tr id="product-'+str.productId+'" data-item="'+str.productId+'" class="each"> \
											<td class="cb"><input type="checkbox" name="cb" value="'+str.productId+'" data-package="'+str.packageType+'" data-price="'+str.originalPrice+'" data-lot="'+str.lotNum+'" data-url="'+str.productUrl+'" data-sale="'+str.salePrice+'" data-category="'+categoryIdForRequest+'" data-date="'+str.validTime+'"></td> \
		...
		*/
        ...
        function aliprice_update_subcategories(category_value) {
            ...
            var categoriesSelect = $("select#categories"), //нужен parent .item-group
                ...
                emptySelect = '<select name="'+ subcategoriesSelectName +'" id="subcategories" class="'+ subcategoriesSelectClass +'"><option value="0" selected="selected"> ---- </option></select>';
                ...
                success: function (data) {
                    ...
                    if(data[0].error) {
                        ...
                        jQuery.each(data, function (index, value) {
                            options += '<option value="'+ value.ali_id +'"> '+ value.name +' </option>';
                        });

                        var notEmptySelect = '<select name="'+ subcategoriesSelectName +'" id="subcategories" class="'+ subcategoriesSelectClass +'">' + options + '</select>';
