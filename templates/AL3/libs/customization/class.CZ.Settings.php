...
class czSettings extends czAdminTpl
{
	...
	function tmplGeneral()
	{
		...
		$this->block( 
			...
			$this->block( 
			$this->row(array(
				$this->uploadImgField( 'ssdma_images_before_footer', array(
						'label'  => __( 'Before Footer', 'ssdma' ),
				) )
	...
	function tmplNav()
	{
        ...
        $this->block( array(
            ...
            $this->row( array(
					...
					$this->dropDownField( 'ssdma_products_cat5', array(
                        'label'=> __('Categories tab 5', 'ssdma'),
                        'value'=> $categories,
                    ))
					))
					));
